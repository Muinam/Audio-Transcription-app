...
export default function Home() {
  ...
  const onDrop = useCallback((accepted: File[], rejected: any[]) => {
    if (rejected.length > 0) {
      setErrorMsg('❌ Sirf audio files accept hoti hain (.mp3, .wav, .m4a, .ogg)')
      setStatus('error')
      return
    }
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      setErrorMsg(detail || '❌ Server se connect nahi ho saka. Backend aur ngrok check karo.')
    }
  }
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Audio ko text mein convert karo using Whisper AI" />
      ...
      <main className="relative z-10 min-h-screen flex flex-col items-center justify-start px-4 py-16">
        ...
        <header className="text-center mb-14 fade-up fade-up-1">
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '1rem', letterSpacing: '0.05em' }}>
            Audio upload karo — AI text mein convert kar dega
          </p>
          ...
          <div style={{
            ...
            <div className="mb-6">
              <label style={{ color: 'var(--text-muted)', fontSize: '0.75rem', letterSpacing: '0.1em', fontFamily: 'Space Mono, monospace' }}>
                STEP 01 — AUDIO FILE UPLOAD KARO
              </label>
                ...
                {file ? (
                  <div>
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: '4px' }}>
                      {formatSize(file.size / 1024)} • Change karne ke liye dobara drag karo
                    </p>
                  </div>
                ) : isDragActive ? (
                  <div>
                    <div style={{ fontSize: '2.5rem', marginBottom: '8px' }}>📂</div>
                    <p style={{ color: 'var(--accent-light)', fontWeight: 600 }}>File yahan drop karo!</p>
                  </div>
                ) : (
                  <div>
                    <div style={{ fontSize: '2.5rem', marginBottom: '12px' }}>🎙️</div>
                    <p style={{ color: 'var(--text)', fontWeight: 600, marginBottom: '6px' }}>
                      Audio file drag karo ya click karo
                    </p>
            ...
            {/* ── Step 2: Transcribe Button ── */}
            <div className="mb-6">
              <label style={{ color: 'var(--text-muted)', fontSize: '0.75rem', letterSpacing: '0.1em', fontFamily: 'Space Mono, monospace' }}>
                STEP 02 — TRANSCRIPTION SHURU KARO
              </label>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: file && status === 'idle'
                    ? '0 0 20px var(--accent-glow), 0 4px 15px rgba(0,0,0,0.3)'
                    : 'none',
                }}
              >
                ...
                {status === 'processing' && (
                  <span className="flex items-center justify-center gap-3">
                    <Waveform />
                    <span>AI Transcribe kar raha hai...</span>
                  </span>
                )}
                {(status === 'idle' || status === 'done' || status === 'error') && '🚀 Transcribe Karo'}
              </button>
